...
@section('title', $estate->title.' İlanı Güncelleniyor')
@section('content')
...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        ...
        @endif
        <form method="post" action="{{route('estates.update',$estate->id)}}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <select name="category" class="form-control" required>
                    ...
                    @foreach($categories as $category)
                        <option @if($article->category_id == $category->id) selected @endif value="{{$category->id}}">{{$category->categoriname}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
